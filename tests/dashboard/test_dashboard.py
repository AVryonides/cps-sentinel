...
async def test_nicegui_page_exposes_complete_explanatory_story() -> None:
    async with user_simulation(build_page) as user:
        ...
        await user.should_see("Digital-twin evidence for cyber-physical anomaly research")
        await user.should_see("Experiment protocol")
        await user.should_see("Dataset boundary")
        await user.should_see("Calibration boundary")
        await user.should_see("Research output")
        ...
        user.find(marker="scenario-battery-actuator-manipulation").click()
        await user.should_see("Battery Command Path", retries=80)
        user.find("Battery health validation").click()
        await user.should_see("Battery degradation", retries=20)
        user.find("SWaT security validation").click()
        await user.should_see("Industrial", retries=20)
